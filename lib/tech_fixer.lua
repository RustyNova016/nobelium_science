...
function Public.add_required_sciences(tech)
    ...
    for _, science in pairs(sciences) do
        if science.nobelium_science ~= nil and science.nobelium_science.ignore_autoadd == false then
            goto continue
        end

        -- Is the tech post promethium and the science pre promethium?
        if tech_stage == constants.science_stages.post_promethium and science.nobelium_science ~= nil and science.nobelium_science.progress_stage == constants.science_stages.pre_promethium then
            -- Then we add
        ...
        if tech_stage == constants.science_stages.post_promethium 
            and science.nobelium_science ~= nil 
            and (
                science.nobelium_science.progress_stage == constants.science_stages.pre_promethium or
                science.nobelium_science.progress_stage == constants.science_stages.pre_nobelium
            ) then
            -- Then we add
            if technology.add_science_to_tech_if_not_set(tech, science.name, 1) then
                output = true
...
function Public.set_nobelium_requirement(tech)
    ...
    -- Replace "promethium-science-pack" requirement for nobelium... If it requires it
    if utils.array_contain(tech.prerequisites or {}, "promethium-science-pack") then
